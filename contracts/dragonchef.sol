...
contract CastleToken is ERC20("KIMCHI.finance", "KIMCHI"), Ownable {
...
contract CastleChef is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        //   pending reward = (user.amount * pool.accCastlePerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accCastlePerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accCastlePerShare; // Accumulated KIMCHIs per share, times 1e12. See below.
    }
    ...
    function pendingCastle(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accCastlePerShare = pool.accCastlePerShare;
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCastlePerShare = accCastlePerShare.add(castleReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accCastlePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 castleReward = multiplier.mul(castlePerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        castle.mint(devaddr, castleReward.div(20)); // 5%
        castle.mint(address(this), castleReward);
        pool.accCastlePerShare = pool.accCastlePerShare.add(castleReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accCastlePerShare).div(1e12).sub(user.rewardDebt);
            safeCastleTransfer(msg.sender, pending);
        }
        pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        user.amount = user.amount.add(_amount);
        user.rewardDebt = user.amount.mul(pool.accCastlePerShare).div(1e12);
    ...
    // Safe castle transfer function, just in case if rounding error causes pool to not have enough KIMCHIs.
    function safeCastleTransfer(address _to, uint256 _amount) internal {
